...
class PressMat(Dataset):
    def __init__(self, annotations_file, img_dir, transform=None, target_transform=None, label2num={"walk":0, "jump":1, "lr_shift":2, "tiptoe":3, "human":4, "no_press":5}):
    ...
    def __getitem__(self, idx):
        ...
        try:
            if np.sum(image[:,:,2]) == 0:
                ...
                # print("same level", np.array(time_image).shape)
            else:
                time_image = []
                for i in range(7):
                    time_image += [image[:,:,i]]
                # print("different level", np.array(time_image).shape)
        except:
            time_image = [image] * 7
            # print("same 2D level", np.array(time_image).shape)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(42240, 128)#changed
