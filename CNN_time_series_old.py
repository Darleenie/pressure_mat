...
DATADIR = "../data/time_series"
TRAIN_TEST_CUTOFF = '2016-04-21' #FIXME
...
for filename in os.listdir(DATADIR):
    ...
    for i in range(len(X[0][0])):
        ...
        for ii in range(len(X)):
            ...
            temp[-1] += [X[ii][jj][i]]
        time_series_temp += temp #2D -> 1D

    # print(np.array(time_series_temp).shape)
    # print(time_series_situp_temp[:7])#len = 7 (valid data)

    for i in range(6):
        # print(time_series_situp_temp[i -10:i])
        time_series += [time_series_temp[i]]
